...
const RegisterEmployeeForm = ({id, email} : {id : string, email : string}) => {
  const [userEmail, setUserEmail] = useState(email)
  const [password, setPassword] = useState("")
  const [isVisible, setIsVisible] = useState(false)
  const createWithEmployeeId = registerEmployee.bind(null, id)
 
  return (
    <form action={createWithEmployeeId} className='bg-rose-500 py-2 px-4 flex flex-col gap-6 w-full rounded-lg'>
      <h1 className='text-center text-3xl font-bold'>Registrar Empleado como usuario</h1>
      ...
      <Button className='bg-rose-700 text-white' type="submit">Registrar Empleado</Button>
    </form>
  )
}
