...
const ChangeEmployeePasswordForm = ({id, email} : {id : string, email : string}) => {
  const [userEmail, setUserEmail] = useState(email)
  const [password, setPassword] = useState("")
  const [isVisible, setIsVisible] = useState(false)
  const changeEmployeePassword = updateEmployeeUser.bind(null, id)
 
  return (
    <form action={changeEmployeePassword} className='bg-rose-500 py-2 px-4 flex flex-col gap-6 w-full rounded-lg'>
